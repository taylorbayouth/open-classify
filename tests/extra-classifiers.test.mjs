...
import { TEST_CATALOG, validClassifierOutputs as validOutputs } from "./fixtures.mjs";
...
test("buildClassifierRegistry throws when an extra collides with a built-in", () => {
  const root = mkdtempSync(join(tmpdir(), "open-classify-extra-"));
  writeClassifier(root, "tools", {
    dispatch_order: 999,
    output_schema: {
      required: ["tags"],
  ...
  assert.throws(
    ...
    (error) =>
      ...
      /duplicate classifier name: tools/.test(error.message),
  );
});
