...
test("route result resolves downstream model from caller config", async () => {
  ...
  assert.equal(result.stop_downstream, false);
  assert.deepEqual(result.handoff.model, {
    key: "reasoning.local_strong",
...
test("downstream model config falls back from exact key to tier", async () => {
  ...
  assert.equal(result.handoff.model.key, "reasoning.local_strong");
  assert.equal(result.handoff.model.model, "local-general");
  assert.equal(result.handoff.model.resolved_from, "local_strong");
});
