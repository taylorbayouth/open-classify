...
// Contributes RAG/memory-search query hints to the route envelope.
const memoryQueriesContribution: Contribution<MemoryRetrievalQueriesResult> = {
  slot: "memory_queries",
  priority: 0,
  build: (result) => (result.queries.length > 0 ? result.queries : undefined),
};
...
> = {
  ...
  ui: {
    label: "Memory Retrieval Queries",
    renderer: "list",
  },
};
