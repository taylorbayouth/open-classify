...
reason semantics:
- reason is a compact diagnostic explanation for the capability-lane choice.
- Keep reason under 200 characters.

Selection guide:
- Classify only the latest user message; earlier messages are context evidence only.
- Choose the specialization that should combine with the separately selected model tier to resolve a concrete downstream model.
